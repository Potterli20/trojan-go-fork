...
conn := value.(any)
conn.(interface{ CloseWithError(code uint32, reason string) error }).CloseWithError(0, "server closed")
return true
})
if s.underlay != nil {
...
for {
conn, err := s.listener.(interface{ Accept(context.Context) (any, error) }).Accept(context.Background())
if err != nil {
select {
case <-s.ctx.Done():
...
defer func() {
conn.(interface{ CloseWithError(code uint32, reason string) error }).CloseWithError(0, "connection closed")
...
go func() {
for {
stream, err := conn.(interface{ AcceptStream(context.Context) (quic.Stream, error) }).AcceptStream(context.Background())
if err != nil {
...
buf := make([]byte, 65536)
for {
n, err := conn.(interface{ ReceiveMessage(context.Context, []byte) (int, error) }).ReceiveMessage(context.Background(), buf)
if err != nil {
